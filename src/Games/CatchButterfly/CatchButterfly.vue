...
export default class CatchButterfly extends Vue {
  ...
  created() {
    document.addEventListener("tobii.point", (e) => {
      ...
      if(((gpoint.x>this.point.x-this.hitBoxSize)&&(gpoint.x<this.point.x+this.hitBoxSize))
      && ((gpoint.y>this.point.y-this.hitBoxSize)&&(gpoint.y<this.point.y+this.hitBoxSize))){
        this.hit()
      }
    });
  }
